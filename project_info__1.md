...
2. `UIExtrasSystem` checks the page’s `data-algo` attribute and, if present, replaces the page body with a standardized algorithm layout.
3. It injects:
...
- Some pages use `data-theme="dark"` and others `data-theme="light"`, which can create inconsistent appearance if the shared CSS expects one default.
